...
`%grep` 是结构化文本提取块。它不直接复刻系统 `grep`、`sed` 或 `awk` 的命令行参数，而是把提取过程组织为文本来源 -> 范围/行选择 -> 字段解析与投影 -> 验证 ->输出。
...
若来源文件同时包含两段 `Center Charge` 表，可以使用 `last` 选择后一段。边界标识必须与来源文本一致；例如，结束标识可以写成不带冒号的 `Sum of charges`。
